...
namespace LCT.Infrastructure.Persistance.ActionsStorage
{
    internal class MongoActionStorageClient : IActionStorageClient
    {
        ...
        public async Task<List<T>> GetActionsAsync<T, TKey>(TKey aggregateId, CancellationToken cancellationToken = default) where T : LctAction<TKey>
        {
            var cursorAsync = await _database.GetCollection<T>($"{typeof(T).Name}").FindAsync(t => t.GroupKey.Equals(aggregateId));
            return await cursorAsync.ToListAsync(cancellationToken);
        }

        public async Task SaveActionAsync<T>(T action, CancellationToken cancellationToken = default) where T : LctAction
            => await _database.GetCollection<T>($"{typeof(T).Name}").InsertOneAsync(action, cancellationToken: cancellationToken);
    }
}
